...
public:
  ...
  inline IndexedNormalSet(const IndexedNormalSet& other)
  {
    _nepsilon = other._nepsilon;
    for (int i=0;i<_grid.size();i++)
    {
      if (_grid[i]!=NULL) delete(_grid[i]);
    }
    _grid.resize(other._grid.size(),NULL);
    for (int i=0;i<other._grid.size();i++)
    {
      if (other._grid[i]==NULL) continue;
      _grid[i] = new AngularGrid(*(other._grid[i]));
    }
    _epsilon = other._epsilon;
    _egSize = other._egSize;
  }

  inline IndexedNormalSet<Point, dim, _ngSize, Scalar>& operator= (const IndexedNormalSet<Point, dim, _ngSize, Scalar>& other)
  {
    _nepsilon = other._nepsilon;
    for (int i=0;i<_grid.size();i++)
    {
