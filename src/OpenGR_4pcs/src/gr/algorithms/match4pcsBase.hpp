...
namespace gr {
    ...
    bool Match4pcsBase<_Functor, TransformVisitor, PairFilteringFunctor, PFO>::SelectQuadrilateral(
        ...
        while (current_trial < MatchBaseType::kNumberOfDiameterTrials) {
            ...
            if (!MatchBaseType::SelectRandomTriangle(base1, base2, base3, sample_pool)){
                continue;
            }

            auto b0 = MatchBaseType::base_3D_[0] = MatchBaseType::sampled_P_3D_[base1];
            auto b1 = MatchBaseType::base_3D_[1] = MatchBaseType::sampled_P_3D_[base2];
            auto b2 = MatchBaseType::base_3D_[2] = MatchBaseType::sampled_P_3D_[base3];
            ...
            if (denom != 0) {
                ...
                #pragma omp parallel for schedule(dynamic) firstprivate(b0, b1, b2)
                for (unsigned int i = 0; i < sample_pool.size(); ++i)
                {
                    const auto &p = MatchBaseType::sampled_P_3D_[sample_pool[i]];
